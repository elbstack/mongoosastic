...
const debug = require('debug')('mongoosastic')
...
function deleteByLocaleIndex (schema, opts) {
  if (!opts) {
    debug('no opts set')
    return
  }
  ...
  if (!opts.index) {
    debug('no index specified')
    return
  }

  /**
   * Makes sure that all other indices get the doc removed.
   */
  const deleteFromIndex = {}

  schema.eachPath(path => {
    const meta = schema.path(path)
    const locale = meta.options.es_locale

    if (!locale) {
      return
    }

    const indexTo = opts.index + '_' + locale
    deleteFromIndex[indexTo] = true
  })

  debug('also delete from indices', Object.keys(deleteFromIndex))
  Object.keys(deleteFromIndex).map(key => {
    deleteByMongoId(Object.assign({}, opts, {
      index: key
    }), nop)
  })
}

function deleteLocaleFromPath (locale, path) {

  if (path.indexOf('_' + locale) === path.length - 3) {
...
function Mongoosastic (schema, pluginOpts) {
  ...
  schema.methods.index = function schemaIndex (inOpts, inCb) {
    ...
    /**
     * Populate several more indices based on a flag on the schema.
     * Use this to fill i18n indices.
     */
    const indexToo = {}
    ...
    schema.eachPath(path => {
      ...
      const indexTo = _opts.index + '_' + locale
      debug('also publish to index', indexTo, path)

      if (!indexToo[locale]) {
        indexToo[locale] = {
          esOptions: Object.assign({}, _opts, {
            index: indexTo,
            body: {} // dont send everything to ES
          })
        }

        if (_opts.body.user) {
          indexToo[locale].esOptions.body.user = _opts.body.user
        }
      }

      // strip the locale from the fieldname
      const indexPath = deleteLocaleFromPath(locale, path)
      indexToo[locale].esOptions.body[indexPath] = _opts.body[path]

      debug(indexToo[locale].esOptions.body, 'body')
    })

    debug(indexToo, 'index values')

    Object.keys(indexToo).map(key => {
      const localeOpts = indexToo[key].esOptions
      esClient.index(localeOpts, (err) => debug('on index hook done', err || 'no error'))
    })
  }
  ...
  schema.methods.unIndex = function unIndex (inOpts, inCb) {
    ...
    if (bulk) {
      ...
      deleteByLocaleIndex(schema, opts)
    }
  }
